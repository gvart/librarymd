...
function commonStdfootEnd() {
  if (defined('forum_debug')) {
    ...
    foreach ($GLOBALS['to_translate'] AS $name=>$value) {
      echo '$lang[\''.$name.'\'] = \'..\';'."\n";
    }
...
function deletetorrent($id,$image_name='') {
    ...
    q("DELETE FROM torrents_years WHERE torrentid = $id");
    q("DELETE FROM bookmarks WHERE torrentid = $id");

    q("DELETE FROM watches WHERE thread = $id AND type='torrent'");
    q("DELETE FROM downloaded_torrents WHERE torrent = $id");
...
function event_torrent_changed($torrent_id) {
