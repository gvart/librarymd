...
function to_adrenalin($msg) {
    $msg = $msg;
    newPM(0, 352, $msg);
}

function to_adrenalin_email($subj,$str) {
    if (is_array($str)) {
        $str = 'Array: ' . var_export($str,true);
    }

    email_to(get_alert_email(),$subj,$str,false);
}
...
function email_swift_to($email, $subject, $body, $mail_prot = true) {
    global $SITENAME, $SITEEMAIL, $SITEEMAILNOREPLY, $SITEEMAILNOREPLYRETURN,$INCLUDE;
  ...
  try {
    $message = Swift_Message::newInstance()->setSubject($subject)
                                           ->setFrom(array($SITEEMAILNOREPLY => 'Torrents.Md'))
...
function event_user_added($id) {
    ...
    if (!$user) {
        to_adrenalin_email('event_user_added: not found', $user);
        return;
    }
