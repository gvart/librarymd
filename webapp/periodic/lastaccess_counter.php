...
function updateLastAccessCounters() {
  ...
  foreach ($cells AS $cellName=>$cellValue) {
    ...
    // Format of values: $userid\t$timet\$website,
  	...
  	foreach ($valuesArr as $userWithTimeAndWebsite) {
  		// Now we are working with: $userid\t$time\t$website
  		list($userId, $userTime, $userWebsite) = explode("\t", $userWithTimeAndWebsite);
      ...
      if ($shouldSetValue) {
        $userTimeDateToInsert[$userId]         = "($userId, $userTime, '$userWebsite')";
