                ...
                <?php
                    ...
                    if ( $showDownloadTorrentLink ) {
                      if (is_file("$torrent_dir/".$torrent['id'].".torrent"))
                        ...
                        echo "Torrentul nu este disponibil, utilizati magnetul ";
                    }

                    $passkeyQParam = User::getPasskeyForDownloadWithQueryParam();

                    $announce_url = get_announce_url($torrent['id']) . $passkeyQParam ;
                    $info_hash_sha1 = unpack("H*",$torrent['latininfo_hash']);
                    $info_hash_sha1 = $info_hash_sha1[1];

                    printf('&nbsp;&nbsp;&nbsp;&nbsp;
                      (<a href="magnet:?xt=urn:btih:%s&dn=%s&tr=%s&tr=%s&tr=%s&tr=%s"
                        class="magnetlnk">magnet</a> -
                          <a href="/forum.php?action=viewtopic&topicid=88073113" target="_blank">?</a>
                      )',
...
$flag_dllist = (isset($_GET["dllist"]))?'&dllist=1':false;
  ...
  $dhtUpdateDateHuman = $torrent['dht_peers_updated'] == '0000-00-00 00:00:00' ? 'niciodată, programat pentru update.' : get_elapsed_time(strtotime($torrent['dht_peers_updated']));
