...
<?php
  $show_versions_activated = isset($_GET["show_versions"]);
  $versioned_descr = false;
  if ($show_versions_activated)
	  $versioned_descr = fetchAll("SELECT changed, descr_ar FROM torrents_details_versioned WHERE id=:id ORDER BY changed DESC LIMIT 8", array('id' => $id) );
?>
<?php if ($show_versions_activated && empty($versioned_descr)): ?>
    <h2><?php echo __("Nici o versiune anterioară n-a putut fi găsită") ?></h2><br/>
<?php endif; ?>
<?php
	if ($versioned_descr && $show_versions_activated) :
?>
<h2><?=__("Versiunile torrentului")?></h2>
<div class="version_control left"><div class="tabs">
  <div class="version_list_title"><?=__("Vezi descrierea salvata la:")?>:</div>
  <?php
	foreach ($versioned_descr AS $key => $descr) {
		echo '<div class="version_head lnk" rel="'. ( intval(strtotime($descr['changed']))+$key ) .'">'. $descr['changed'] .'</div>';
	}

	echo '</div><div class="content">';

	foreach ($versioned_descr AS $key => $descr) {
		echo '<div class="version_row" rel="'. ( intval(strtotime($descr['changed']))+$key ) .'">';

		foreach (unserialize($descr['descr_ar']) AS $desc_key => $desc_row) {
			echo '<span class="rowspan'. ( ($desc_key == 'additional')?' additional':'' ) .'">';

			if ( $desc_key == 'additional')
				echo '<b>'. esc_html($desc_row[0][0]) .'</b> :&nbsp;&nbsp;&nbsp;<span>' . esc_html($desc_row[0][1]) . '</span><br>';
			else
				echo '<b>'. esc_html($desc_key) .'</b> :&nbsp;&nbsp;&nbsp;<span>' . esc_html($desc_row) . '</span><br>';

			echo '</span>';
		}
		echo '<div class="restore center">[<span class="lnk">Aplica descrierea afisata mai sus</span>]</div>';
		echo '<div class="nota_bene"><strong>Nota!</strong> Dupa ce ai aplicat o descriere mai veche, o poti edita in formularul de mai jos, cand e gata, pur si simplu dati click pe Edit it!</div>';
		echo '</div>';
	}
...
<?php endif;?>
<?php
//asta posibil ar trebui deja de comentat, pentru că-l ridic mai sus..
/*
if (!isset($CURUSER) || ($CURUSER["id"] != $row["owner"] && get_user_class() < UC_MODERATOR && get_user_class() != UC_SANITAR)) {
	barkk(__('Nu sunteți proprietarul acestui torrent'));
}
...
<a href="?id=',$id,'&show_versions=1">['.__("Arată versiunile descrierii").']</a>
</div><br/>';
