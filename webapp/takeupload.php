...
//Torrent name
$sql['torrent_name'] = _esc($t_details['name']); // The only one html escaped value
...
$ret = q("INSERT INTO torrents (name, image, filename, owner, visible, size, numfiles, type, category, save_as, added, last_action, team) VALUES (" .
		$sql['torrent_name'].', ' . $sql['image'] . ', '.implode(",", array_map("sqlesc", array($fname, $CURUSER["id"], "no", $totallen, count($filelist), $type, (int)$t_details['type'], $dname))) .
		", '" . get_date_time() . "', '" . get_date_time() . "','$teamRelease')");
...
$torrent_details_sql = "INSERT INTO torrents_details (id,descr_ar,descr_search_str,descr_html)
      VALUES(".(int)$id.", ".$sql['desc_ar'].", ".$sql['desc_search_str'].", ".$sql['desc_details_html'] . ")";
...
event_torrent_changed($id);

$searchindex = $t_details['search_sql']; //All fields are already escaped, preparated for sql insert
$searchindex['year'] = (int)$searchindex['year'];
$searchindex['lang'] = (int)$searchindex['lang'];
q("INSERT INTO searchindex (id,name,year,lang,genre)
		    VALUES ($id,$searchindex[name],$searchindex[year],$searchindex[lang],$searchindex[genre])");
