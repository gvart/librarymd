...
foreach ($threads AS $thread) {
	...
	elseif ($thread['type'] == 'torrent' && !isTopicmd()) {
        ...
        /*$res = q("SELECT comments.id, comments.user, comments.added, users.username AS lpusername, users.gender AS lpuserGender
        		  FROM comments
        		  LEFT JOIN users ON comments.user = users.id
        		  WHERE torrent=$torrentid ORDER BY comments.id DESC LIMIT 1");

        $arr = mysql_fetch_assoc($res); // unused query %) */
